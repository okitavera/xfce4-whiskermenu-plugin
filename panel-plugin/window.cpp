...
WhiskerMenu::Window::Window(Plugin* plugin) :
	...
	m_profile_layout_alternate(false)
{
	m_geometry.x = 0;
	m_geometry.y = 0;
...
void WhiskerMenu::Window::show(const Position position)
{
	...
	if (m_profile_layout_alternate != wm_settings->profile_layout_alternate)
	{
		m_profile_layout_alternate = wm_settings->profile_layout_alternate;

		g_object_ref(m_profilepic->get_widget());
		g_object_ref(m_username);
		if (m_profile_layout_alternate)
		{
